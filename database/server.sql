...
CONSTRAINT CLS_NUM_SCH_NAME_RELATE_TO_ROLE FOREIGN KEY (CLASS_NUMBER, SCHOOL_NAME)
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
CONSTRAINT ROLE_RELATE_TO_CLS FOREIGN KEY (ROLE_ID)
    REFERENCES I_ROLE (ROLE_ID)
    ON DELETE CASCADE
    ON UPDATE CASCADE
);

-- represents a group of class roles, used for menu
CREATE TABLE CLASS_ROLE_GROUP
(MENU_GROUP_NAME VARCHAR(50) NOT NULL,
 ...
 CONSTRAINT GROUP_IN_SERVER FOREIGN KEY (SERVERID)
     ...
     ON DELETE CASCADE
     ON UPDATE CASCADE,
 CONSTRAINT CLASS_ROLE_GROUP_RELATION
     FOREIGN KEY (CLASS_NUMBER, SCHOOL_NAME, ROLE_ID)
    REFERENCES CLASS_ROLE (CLASS_NUMBER, SCHOOL_NAME, ROLE_ID)
    ON DELETE CASCADE
    ON UPDATE CASCADE
);
...
(MENUMSGID VARCHAR(20) PRIMARY KEY NOT NULL,
 ...
 CONSTRAINT MENU_BELONG_TO_CLASS_ROLE_GROUP FOREIGN KEY (MENU_GROUP_NAME, SERVERID)
     REFERENCES CLASS_ROLE_GROUP (MENU_GROUP_NAME, SERVERID)
     ON DELETE CASCADE
     ON UPDATE CASCADE
);
